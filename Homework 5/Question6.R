...
library(mixtools)
data = faithful
data$waiting = data$waiting/100
data$eruptions = data$eruptions/10
data
...
calculate_z_vals <- function(x_vals,y_vals,pis,mus,sigmas,k){
  count <- 1
  ...
  for(i in 1:100){
    ...
    for(j in 1:100){
      for(K in 1:k){
        sum <- sum + pis[K]*dmvnorm(c(x_vals[i],y_vals[j]),mus[[k]],sigmas[[k]])
      }
    }
    z_vals[count] <- sum
    count = count + 1
...
sigmas[[1]]
z_vals = calculate_z_vals(x_vals, y_vals, pis, mus, sigmas, k)
fig <-  plot_ly()
fig <- add_trace(
  fig,
  type = 'scatter',
